...
import com.example.zebrascannerpoc.helpers.AvailableScanner;
import com.example.zebrascannerpoc.helpers.Barcode;
import com.example.zebrascannerpoc.helpers.Constants;
import com.example.zebrascannerpoc.helpers.Foreground;
...
import static com.example.zebrascannerpoc.helpers.Constants.VIRTUAL_TETHER_HOST_BACKGROUND_MODE_NOTIFICATION;
import static com.example.zebrascannerpoc.helpers.Constants.VIRTUAL_TETHER_HOST_NOTIFICATION_CHANNEL_ID;
import static com.example.zebrascannerpoc.helpers.Constants.logAsMessage;
...
//                    Application.barcodeData.add(barcode);
                    for (ScannerAppEngine.IScannerAppEngineDevEventsDelegate delegate : mDevEventsDelegates) {
                        if (delegate != null) {
                ...
                case Constants.BARCODE_RECEIVED:
                    ...
                    if (!Application.isFirmwareUpdateInProgress) {
                        if (Application.currentScannerId != Application.SCANNER_ID_NONE) {
                            Intent intent = new Intent(".ActiveScannerActivity");
                ...
                case Constants.SESSION_ESTABLISHED:
                    ...
                    /* notify connections delegates */
                        for (ScannerAppEngine.IScannerAppEngineDevConnectionsDelegate delegate : mDevConnDelegates) {
                            if (delegate != null) {
                    ...
                    if (Application.mDevListDelegates != null) {
                        for (ScannerAppEngine.IScannerAppEngineDevListDelegate delegate : Application.mDevListDelegates) {
                            if (delegate != null) {
    ...
    public void dcssdkEventScannerDisappeared(int i) {
        dataHandler.obtainMessage(Constants.SCANNER_DISAPPEARED, i).sendToTarget();
        int scId = Application.currentScannerId;
        virtualTetherHostAlarm(scId);
    }
    ...
    public void dcssdkEventCommunicationSessionTerminated(int i) {
        dataHandler.obtainMessage(Constants.SESSION_TERMINATED, i).sendToTarget();
    }

    @Override
    public void dcssdkEventBarcode(byte[] bytes, int i, int i1) {
        Barcode barcode = new Barcode(bytes, i, i1);
        dataHandler.obtainMessage(Constants.BARCODE_RECEIVED, barcode).sendToTarget();
    }

    @Override
    public void dcssdkEventImage(byte[] bytes, int i) {
        dataHandler.obtainMessage(Constants.IMAGE_RECEIVED, bytes).sendToTarget();
    }

    @Override
    public void dcssdkEventVideo(byte[] bytes, int i) {
        dataHandler.obtainMessage(Constants.VIDEO_RECEIVED, bytes).sendToTarget();
    }

    @Override
    public void dcssdkEventBinaryData(byte[] bytes, int i) {
        logAsMessage(TYPE_DEBUG, TAG, "BinaryData Event received no.of bytes : " + bytes.length + " for Scanner ID : " + i);
    }
    ...
    public void dcssdkEventAuxScannerAppeared(DCSScannerInfo dcsScannerInfo, DCSScannerInfo dcsScannerInfo1) {
        dataHandler.obtainMessage(Constants.AUX_SCANNER_CONNECTED, dcsScannerInfo1).sendToTarget();
    }
...
//                            vibrator.vibrate(pattern, 0);
                        ...
                        Application.virtualTetherHostActivated = true;
